...
public class TraversalProgram {
	...
	private static final int MIN_CITIES = 25; // minimum number of nodes
												// (cities) to have so the graph
												// does not become disconnected
	private static final int MAX_CITIES = 51; // maximum number of nodes
												// (cities) we can have
	private City[] cities; // This array will be initialised with City objects
							// by initialiseCities()
	private int routeCounter; // Used by initialiseCities() to count routes
	private int[] whiteCities; // array to hold the indices (in the cities
								// array) or white (unprocessed) cities. A -1
								// indicates that the city with this index is no
								// longer white
	private int nextWhiteCity; // used in getNextWhiteCity below, gives the
								// index of the next white city in whiteCities
	private int maxCities; // number of cities to initialise
	private int steps; // use this field to count the number of program steps
						// (number of nodes put on stack or in queue, plus
						// number of comparison steps in sorting, where
						// applicable)
	...
	private int getNextWhiteCity() {
		while ((nextWhiteCity < whiteCities.length)
				&& (whiteCities[nextWhiteCity] < 0)) {
			nextWhiteCity++;
		}
		...
		while (!cityStack.isEmpty()) {
			...
			if (cityStack.getStackTop().getLastRouteChecked() < routeLength - 1) {
				cityStack.getStackTop().setLastRouteChecked( // set next route
						...
						cityStack.getStackTop().getLastRouteChecked() + 1);
				System.out
						.println("Checking route to "
								+ cities[cityStack.getStackTop()
										.getSortedRoutes()[cityStack
										.getStackTop().getLastRouteChecked()]
										.getDestination()].getName());
				System.out.println("Incrementing steps from" + (steps) + "to"
						+ (steps + 1));
			...
			if (city.getPathTo() == "") {
				...
				for (int j = 0; j < routes.length; j++) {
					...
					System.out.println("    Route to: "
							+ cities[route.getDestination()].getName()
							+ " for $" + route.getCost());
				}
			} else {
				System.out.println(" ($" + city.getCostTo() + ")"
						+ city.getPathTo());
			}
			System.out.println();
		}
		System.out.println(cities.length + " cities and " + routeCounter
				+ " routes loaded.");
		System.out.println();
	}
	...
	public void start() {
		...
		while (choice != 0) {
			...
			while ((maxCities < MIN_CITIES) || (maxCities > MAX_CITIES)) {
				System.out.print("Number of cities to initialize (25 to "
			...
			System.out
					.println("4) Perform DFS with path optimisation and cheapest edge first");
			...
			System.out
					.println("8) Perform BFS with path optimisation and cheapest edge first");
			System.out
					.println("9) Perform the Dijkstra algorithm (optional - you do not need to implement this)");
